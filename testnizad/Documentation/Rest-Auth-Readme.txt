...
-------
     ...
     * Running on  http://srv.kemoke.net:5000/
...
POST /api/users
    ...
    On success a status code 201 is returned. The body of the response contains a JSON object with the newly added user and user_id. A `Location` header contains the URI of the new user.
...
- PUT /api/users/<int:id>
    Register a new user.
    The body must contain a JSON object that defines `username` or `password` fields.
    On success a status code 201 is returned. The body of the response contains a JSON object with the newly added user and user_id. A `Location` header contains the URI of the new user.
    On failure status code 400 (bad request) is returned.
    Notes:
    - The password is hashed before it is stored in the database. Once hashed, the original password is discarded.
...
- DELETE /api/users/<int:id>
    ...
    Delete user by id.
    On success a status code 202 is returned. The body of the response contains a JSON object with the message: "User successfully deleted!".
    On failure status code 400 (bad request) is returned.
...
The following `curl` command registers a new user with username `taba` and password `python`:

    $ curl -i -X POST -H "Content-Type: application/json" -d '{"username":"taba","password":"python"}' 

http://test.p3rsonal.com/api/users
...
These credentials can now be used to access protected resources:
    ...
    $ curl -u taba:python -i -X 

GET http://test.p3rsonal.com/api/resource
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 30
    Server: Werkzeug/0.9.4 Python/2.7.3
...
Using the wrong credentials the request is refused:
    ...
    $ curl -u taba:ruby -i -X GET http://test.p3rsonal.com/api/resource
...
Finally, to avoid sending username and password with every request an authentication token can be requested:
    ...
    $ curl -u taba:python -i -X GET http://test.p3rsonal.com/api/token
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 139
    Server: Werkzeug/0.9.4 Python/2.7.3
...
And now during the token validity period there is no need to send username and password to authenticate anymore:
    ...
    $ curl -u eyJhbGciOiJIUzI1NiIsImV4cCI6MTM4NTY2OTY1NSwiaWF0IjoxMzg1NjY5MDU1fQ.eyJpZCI6MX0.XbOEFJkhjHJ5uRINh2JA1BPzXjSohKYDRT472wGOvjc:x -i -X 
